...
interface DailyStandardEntryDao {
    ...
    @Query("""
        SELECT * FROM daily_standard_entries 
        WHERE uID = :uID AND date >= :startOfDay AND date < :endOfDay 
        ORDER BY roleId ASC, standardType ASC
    """)
    ...
    @Query("""
        SELECT * FROM daily_standard_entries 
        WHERE uID = :uID AND isCompleted = 0 AND date >= :startOfDay AND date < :endOfDay
        ORDER BY roleId ASC
    """)
    ...
    @Query("""
        SELECT * FROM daily_standard_entries 
        WHERE uID = :uID AND roleId = :roleId AND date >= :startOfDay AND date < :endOfDay
    """)
    fun observeForDayByRole(
        uID: String, roleId: String, startOfDay: Long, endOfDay: Long
    ): Flow<List<DailyStandardEntryEntity>>

    // Marcar completado manualmente
    @Query("""
        UPDATE daily_standard_entries 
        SET isCompleted = 1, completedAt = :completedAt, xpAwarded = :xpAwarded, isSynced = 0
        WHERE id = :entryId
    """)
    ...
    @Query("""
        ...
        WHERE uID = :uID AND standardType = 'TRAINING'
        AND date >= :startOfDay AND date < :endOfDay
        AND isCompleted = 0
    """)
    suspend fun autoValidateTraining(
        uID: String, startOfDay: Long, endOfDay: Long, completedAt: Long, xpAwarded: Int
    )
    ...
    @Query("""
        ...
        AND date >= :startOfDay AND date < :endOfDay
    """)
    suspend fun getIncompleteForDay(
        uID: String, startOfDay: Long, endOfDay: Long
    ): List<DailyStandardEntryEntity>
    ...
    @Query("""
        SELECT standardId FROM daily_standard_entries 
        WHERE uID = :uID AND date >= :startOfDay AND date < :endOfDay
    """)
    suspend fun getExistingStandardIdsForDay(
        uID: String, startOfDay: Long, endOfDay: Long
    ): List<String>
}